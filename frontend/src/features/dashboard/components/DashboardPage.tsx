...
export function DashboardPage() {
  ...
  const getActionText = (log: any) => {
    ...
    if (entityType === 'TASK') {
      if (action === 'CREATE') return `${t('dashboard.activity.created_task')} "${changes.title}"`;
      if (action === 'UPDATE')
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button
              ...
              onClick={() => navigate('/projects')}
            >
              <Plus className="mr-2 h-4 w-4 stroke-[3]" /> {t('dashboard.new_project')}
            </Button>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 flex flex-col gap-6 min-h-0 min-w-0">
          ...
          <div className="flex-1 flex flex-col min-h-0 space-y-4">
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar min-h-0">
              <RecentProjects projects={projects} isLoading={isLoadingProjects} />
            </div>
          </div>
        </div>
