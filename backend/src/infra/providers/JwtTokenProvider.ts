...
export class JwtTokenProvider implements ITokenProvider {
  generateAccessToken(payload: TokenPayload): string {
    return jwt.sign({ tenantId: payload.tenantId, role: payload.role }, env.JWT_SECRET, {
      subject: payload.sub,
      expiresIn: env.JWT_EXPIRES_IN,
    } as jwt.SignOptions);
  }
  generateRefreshToken(payload: TokenPayload): string {
    return jwt.sign({ tenantId: payload.tenantId, role: payload.role }, env.JWT_REFRESH_SECRET, {
      subject: payload.sub,
