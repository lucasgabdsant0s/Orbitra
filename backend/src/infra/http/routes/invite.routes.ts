...
import { acceptInviteUseCase, createInviteUseCase, verifyInviteUseCase } from '../../container.js';
...
export async function inviteRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(
    ...
    {
      schema: {
        params: tokenParamSchema.merge(
          idParamSchema.omit({ id: true }).extend({ tenantId: idParamSchema.shape.id }),
        ),
        tags: ['Invites'],
      },
    },
  ...
  app.post(
    '/tenants/:tenantId/invites/:token/accept',
    {
      schema: {
        params: tokenParamSchema.merge(
          idParamSchema.omit({ id: true }).extend({ tenantId: idParamSchema.shape.id }),
