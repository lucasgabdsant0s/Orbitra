...
export async function commentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/projects/:projectId/comments', { 
    schema: { body: createCommentSchema, params: projectIdParamSchema, tags: ['Comments'] },
    ...
    const result = await createCommentUseCase.execute(tenantId, projectId, userId, body);
    return reply.status(201).send(result);
  });

  app.get('/projects/:projectId/comments', {
    schema: { params: projectIdParamSchema, query: paginationSchema, tags: ['Comments'] },
    ...
    const result = await listCommentsUseCase.execute(tenantId, projectId, { page, limit }, { parentId });
    return reply.status(200).send(result);
  });

  app.patch('/comments/:id', {
    schema: { params: idParamSchema, body: updateCommentSchema, tags: ['Comments'] }
  }, async (request, reply) => {
