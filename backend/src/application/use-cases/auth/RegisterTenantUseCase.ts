...
export class RegisterTenantUseCase {
  ...
  async execute(input: RegisterTenantInput): Promise<RegisterTenantOutput> {
    ...
    const tenant = await this.tenantRepository.create(new Tenant({ name: input.tenantName, slug }));
    const existingUser = await this.userRepository.findByEmailGlobal(input.email);
    if (existingUser) {
